...
func TestRegressionTree(t *testing.T) {
	...
	predicted := Predict1(testData,train1(trainData, target))
	// predicted.Plot()
	...
	myModelAcc := metrics.MeanSquareError(actualLabels, predicted)
	skLearnModelAcc := metrics.MeanSquareError(expectedPredictions, actualLabels)

	if !testutils.IsAcceptableAccuarcyDiff(myModelAcc, skLearnModelAcc, 80) {
		t.Error("Error accuarcy is not acceptable. This Model accuarcy is: ", myModelAcc, "but sklearn model accuarcy is :", skLearnModelAcc)
	}
}
